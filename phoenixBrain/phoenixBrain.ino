...
#define BASELINE_INDICATOR_PIN  13

int rollInitialValues = 0;
int pitchInitialValues = 0;
int yawInitialValues = 0;
...
int magnitudeHydrophone(int hydrophoneNumber){
    return 6; 
}

void setup()
{
  //Setup LED on pin 13 as indicator of when baselining is complete:
  pinMode(BASELINE_INDICATOR_PIN, OUTPUT);
...
{  
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    yaw = yaw - yawBaseline;
    pitch = pitch - pitchBaseline;
    roll = roll - rollBaseline; 
    
    if (startUpCounter <= READING_TIME){
        yawInitialValues = int (yaw + yawInitialValues);
        pitchInitialValues = int( pitch + pitchInitialValues);
        rollInitialValues = int(roll + rollInitialValues);
        startUpCounter++;
    }
    if (startUpCounter == READING_TIME){
        yawBaseline = int ( float(yawInitialValues) / READING_TIME );
        pitchBaseline = int ( float(pitchInitialValues) / READING_TIME );
        rollBaseline = int ( float(rollInitialValues) / READING_TIME );
        
        //Notify user that baselining is complete!
        digitalWrite(BASELINE_INDICATOR_PIN, HIGH);
        Serial.println("Debiased roll, pitch & Yaw!");
    }
    ...
    if(SerialDebug) {
        ...
        //Setpoint for pitch is always zero to keep robot level
        int pitchGain = computePorportionalGain(0, KP_PITCH, pitch); 
        Serial.print("Pitch gain is: "); Serial.println(pitchGain);
        //(int setPoint, int gain, float actualPoint)
          /* #define KP_ROLL 2
    #define KP_PITCH 2
    #define KP_YAW 2*/
    }
